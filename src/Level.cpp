...
//        std::cout << "Error music in level" << std::endl;
    ...
    for(int i = 0; i< (sizeof(this->builders)/sizeof(this->builders[i])); i++) {
    ...
    for(int i = 0; i< (sizeof(this->skies)/sizeof(this->skies[i])); i++) {
    ...
    for(int i = 0; i < (sizeof(this->blocks)/sizeof(this->blocks[i])); i++) {
