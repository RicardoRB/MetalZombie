...
Player::Player(char file_texture[], float _windowWidth) : Character(file_texture) {
    //Add walk animation
    ...
    //Shot
    this->shot = new Shot(texture,(char*)"res/sounds/effects/shots/shot.ogg");
}

Player::Player(sf::Texture* _texture, float _windowWidth) : Character(_texture) {
    //Add walk animation
    for(int i=112; i<433; i=i+64) {
        walk_frames.addFrame(1.f, sf::IntRect(i, 350.f, 54.f, 55.f));
    }
...
void Player::moveRight() {
    ...
    if(this->posWindowX <= (this->windowWidth/2) || endLevel) {
        if(posWindowX <= windowWidth) {
            this->setVelX(120.f);
        }
    } else {
...
bool Player::isEndLevel() {
    return this->endLevel;
}

void Player::setEndLevel(bool _endLevel) {
    this->endLevel = _endLevel;
}
